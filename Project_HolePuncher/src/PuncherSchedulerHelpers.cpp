...
uint32_t PuncherScheduler::calcMotorSpeedPulse(int32_t length10UM, uint16_t length_type, int32_t speed10UMpS, int32_t microSteps)
{
    ...
    res += 0.5;
    ESP_LOGI("SchedulerHelper", "pulse %ld", (int32_t)res);
    return (int32_t)res;
}
...
{
    return calcMotorSpeedPulse(std::any_cast<int32_t>(length10UM), std::any_cast<uint16_t>(length_type), std::any_cast<int32_t>(speed10UMpS), std::any_cast<int>(microSteps));
}